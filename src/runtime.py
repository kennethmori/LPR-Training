...
from src.services.runtime_payload_service import RuntimePayloadService
...
def make_payload_handler(
    app: FastAPI,
    payload_service: RuntimePayloadService,
    role: str,
) -> Callable[[dict[str, Any]], None]:
    def handler(payload: dict[str, Any]) -> None:
        session_result = payload_service.process_payload(payload)
...
def build_camera_runtime(app: FastAPI, services: CoreServices, settings: dict[str, Any]) -> CameraRuntime:
    camera_services: dict[str, CameraService] = {}
    payload_service = RuntimePayloadService(services)
    for role, role_settings in services.camera_settings_map.items():
        merged_settings = _merged_camera_settings(settings, services, role_settings)
        ...
        camera_services[role] = CameraService(
            ...
            camera_role=role,
            source_name=source_name,
            on_payload=make_payload_handler(app, payload_service, role),
